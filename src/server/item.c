...
void item_collision(entity *item, entity *ent)
{
    ...
    if( new_health > 200 ) //used to be et->health
        new_health = 200; /* keep new_health <= et->health */
    ...
    break;
    case FULLHEAL:
    if( ent->health == 200 ) { /* Already at 200 */
	    remove_item = 0;
	    break;
	}
    ent->health = 200; /* boost to 200!*/
	break;
    case GIVEWEAPON:
	if( it->item_value >= num_weapon_types ) {
	    printf("ERROR, item has value %d, which is >= num_weapon_types!\n",
		   it->item_value);
    } else if( (game.mode == HOLYWAR||game.mode==TEAMDEATHMATCH) && ent->has_weapon[it->item_type] ) {
	    remove_item = 0;
	} else {
    ...
    if( remove_item ) {
    if( (game.mode == HOLYWAR||game.mode==TEAMDEATHMATCH) && it->item_respawn_time ) {
	    item->mode = LIMBO;

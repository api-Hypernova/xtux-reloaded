...
void entity_killed(entity *shooter, entity *victim, point P, byte weaptype)
{
    ...
    victim->health = 0; /* Just to make sure */ //take this away so I can check negative health for gib effects ;)
    victim->frame = 0;
//    if(victim->health<-29)
//    {
...
//    }
	...
	victim->frags--;
	if( victim->name ) {
        snprintf(buf, TEXTMESSAGE_STRLEN, "%s couldn't take it anymore",victim->name);
	...
	case SAVETHEWORLD:
	    ...
	    break;
    case TEAMDEATHMATCH:
        if( victim->class == NEUTRAL || (victim->color1==shooter->color1 ))
        change--;
        else {
        change++;
        /* OBITUARY IF CLIENT?? */
...
void entity_animate(void)
{
	...
	if( animate ) {
        if( framelen ) //&& ent->health>-30) //don't animate if hp less than -30

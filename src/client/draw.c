...
static void draw_health_bar(int health)
{
    ...
    clear_area(win.status_buf, BAR_XOFF, BAR_YOFF,
           health, CAFF_BAR_H, health>100?"blue":"white");
...
void draw_status_bar(void)
{
    ...
    win_print(win.status_buf, str, 0, 0, 3, client.health>100?"blue":"white");
    }
    ...
    if(client.gamemode==HOLYWAR)gamemode="Deathmatch";
    else if(client.gamemode==TEAMDEATHMATCH)gamemode="Team Deathmatch";
    else if(client.gamemode==SAVETHEWORLD)gamemode="Save The World";
    snprintf(str, 48, "Map: %s, Mode: %s", map->name, gamemode);
    win_print(win.status_buf, str, text_start, 0, 2, "white");
    
    snprintf(str, 48, "Frags: %4d, Team ID %d", client.frags, client.color1);

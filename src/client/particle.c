...
#define RAIL_TIME .5//1 /* Seconds rail trail stays up */
/* OP = Outer particles */
#define OP_SPACING 2
#define OP_FREQ 35
...
{
    if(system("/usr/local/bin/mpg123 -f 1500 data/sounds/nexfire.mp3 >&/dev/null &"))printf("Error playing railgun sound!\n");
    particles_t *ptl;
    msec_t expire;
           ...
           netushort length, byte color1, byte color2)
{
    ...
    /* Create the INNER particles */
    ptl = particles_new(COL_YELLOW, PTL_TOP, num_ip);
    ptl->expire = expire;
    for( i=0 ; i < num_ip ; i++ ) {
    ptl->pos[i].x = src_x + x_d * i * IP_SPACING;
    ptl->pos[i].y = src_y + y_d * i * IP_SPACING;
...
#define INNER COL_BLACK
#define OUTER COL_ORANGE
void explosion_init(byte dir, netushort src_x, netushort src_y,
		    netushort length, byte color1, byte color2)
{
    if(system("/usr/local/bin/mpg123 -f 1500 data/sounds/rocket_impact.mp3 >&/dev/null &"))printf("Error playing explosion sound!\n");
    particles_t *ptl;
    msec_t expire;
    float vel, f, inc;
    int i;

    expire = gettime() + M_SEC/2;
         ...
         byte color1, byte color2)
{
    ...
    /* "Extra" particles (ie bullet holes/stationary d) */
#define NUM_HOLEPTLS 35//16
    if( client.ep_expire ) {
	if( color2 == COL_GRAY )
	    color2 = COL_BLACK;
	...
	if( client.ep_expire < 0 )
        ptl->expire = (color2==COL_BLACK?gettime()+1000:ULONG_MAX); /* Should never expire */ //let weird bulletholes disappear earlier :)
	else
		...
		byte color1, byte color2)
{
    ...
    for(j=0; j<5; j++) {
    shards_init(dir, src_x + (rand()%15) - 3, src_y+ (rand()%15) - 3, num * 2, COL_RED, COL_RED);
    }
...
void spawn_init(byte dir, netushort src_x, netushort src_y, netushort num,
		byte color1, byte color2)
{
    if(system("/usr/local/bin/mpg123 -f 1500 data/sounds/itemspawn.mp3 >&/dev/null &"))printf("Error playing spawn sound!\n");
    particles_t *ptl;
    msec_t expire;
    unsigned int i, num_particles;
    float f, inc, vel, rad;
...
void teleport_init(byte dir, netushort src_x, netushort src_y, netushort num,
		byte color1, byte color2)
{
    if(system("/usr/local/bin/mpg123 -f 1500 data/sounds/itemspawn.mp3 >&/dev/null &"))printf("Error playing teleport sound!\n");
    particles_t *ptl;
    msec_t expire;
    unsigned int i, num_particles;
    float f, inc, vel, rad;
